...
// Description: Show any text as a QR code on a TurboWarp sprite.
// By: Hiroya Kubo
// License: MPL-2.0
...
(function (Scratch) {
  ...
  var block_definitions_default = {
  	extensionName: "QR Display",
  	blocks: [
  		{
  			"opcode": "showQrCode",
  			"blockType": "COMMAND",
  			"text": "show [TEXT] as QR code with error correction [LEVEL]",
  			"description": "Shows the text as a QR code in place of this sprite's costume. The costume itself is not changed and comes back when the QR code is hidden or the project stops.",
  			"arguments": {
  				"TEXT": {
  					"type": "STRING",
  		...
  		},
  		{
  			...
  			"blockType": "COMMAND",
  			"text": "hide QR code",
  			"description": "Puts this sprite's costume back in place of its QR code.",
  			"arguments": {}
  		},
  		{
  			"opcode": "isShowingQrCode",
  			"blockType": "BOOLEAN",
  			"text": "showing QR code?",
  			"description": "Reports whether this sprite is showing a QR code. Switching costume takes the QR code off.",
  			"arguments": {}
  		},
  		{
  			...
  			"blockType": "REPORTER",
  			"text": "QR code error",
  			"description": "Reports why the last show block of this extension failed, such as text-too-long, or nothing when it succeeded.",
  			"arguments": {}
  		}
  	],
  	menus: { "errorCorrectionLevels": {
  ...
  var QrDisplayError = class extends Error {
  ...
  var QR_ERROR_CORRECTION_LEVELS = [
  	"L",
  	"M",
  	"Q",
  	"H"
  ];
  function parseErrorCorrectionLevel(value) {
  	const level = String(value ?? "").trim().toUpperCase();
  	if (QR_ERROR_CORRECTION_LEVELS.includes(level)) return level;
  ...
  function createQrSymbol(text, level = "M") {
  	if (text === "") throw new QrDisplayError("empty-text", "There is no text to encode.");
  	let qr;
  	try {
  		qr = import_browser.create([{
  			data: new TextEncoder().encode(text),
  			mode: "byte"
  		}], { errorCorrectionLevel: level });
  	} catch (error) {
  		throw new QrDisplayError("text-too-long", `The text does not fit in one QR code at error correction level ${level}.`, { cause: error });
  	}
  	return {
  		version: qr.version,
  ...
  */
  var SpriteSkinDisplay = class {
  	constructor(runtime) {
  		this.runtime = runtime;
  		this.displays = /* @__PURE__ */ new Map();
  	}
  	validateTarget(target) {
  		if (!target || target.isStage) throw new QrDisplayError("invalid-target", "A QR code can only be shown on a sprite.");
  		if (target.isOriginal === false) throw new QrDisplayError("invalid-target", "A QR code cannot be shown on a clone.");
  	...
  	*/
  	show(targetValue, svg, rotationCenter) {
  		...
  		try {
  			...
  			throw new QrDisplayError("renderer-unavailable", "The renderer could not show the QR skin.", { cause: error });
  		}
  	...
  	hide(target) {
  		...
  		try {
  			if (this.runtime.targets?.includes(target) !== false && renderer?.updateDrawableSkinId) {
  	...
  	isShowing(target) {
  		...
  		return skinId === void 0 || skinId === record.temporarySkinId;
  	}
  };
  function requireRenderer(renderer) {
  	if (!renderer || typeof renderer.createSVGSkin !== "function" || typeof renderer.destroySkin !== "function" || typeof renderer.updateDrawableSkinId !== "function") throw new QrDisplayError("renderer-unavailable", "The TurboWarp renderer is not available.");
  	return renderer;
  }
  function drawableSkinId(renderer, drawableId) {
  	const skinId = renderer._allDrawables?.[drawableId]?._skin?._id;
  	return Number.isInteger(skinId) && Number(skinId) >= 0 ? Number(skinId) : void 0;
  }
  function costumeSkinId(target) {
  	const costumes = target.getCostumes?.();
  	const index = target.currentCostume;
  	if (!costumes || typeof index !== "number") return void 0;
  ...
  */
  var QrDisplayExtension = class {
  	constructor(runtime = Scratch.vm?.runtime ?? {}) {
  		...
  		this.targetRemovedListener = (target) => {
  			if (typeof target === "object" && target !== null) this.display.hide(target);
  		};
  		this.runtime = runtime;
  ...
  Scratch.extensions.register(new QrDisplayExtension());
  //#endregion
